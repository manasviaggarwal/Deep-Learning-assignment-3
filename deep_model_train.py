...
def data_processing(unique_words):
  mat = np.load(glv + '.npy')
  embed = Embedding(unique_words, 300, weights=[mat], input_length=42, trainable=True)
  # embed = Embedding(unique_words, 300, weights=[embd], input_length=42, trainable=True)
  arguments ={}
...
def model_training(model,trainset,testset,valset):
  ...
  model.fit([trainset[0], trainset[1]], trainset[2], batch_size=512, nb_epoch=1, validation_data=([valset[0], valset[1]], valset[2]), callbacks=callbacks)
  model.load_weights(tmpfn)

  loss, acc = model.evaluate([trainset[0], trainset[1]], trainset[2], batch_size=512)
  print('Train loss:  ',loss, 'Train accuracy:  ',acc)
  loss, acc = model.evaluate([valset[0], valset[1]], valset[2], batch_size=512)
  ...
  p=model.predict([testset[0], testset[1]])
  ...
  with open("model/deep_model_json.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
  model.save_weights("model/deep_model_weights.h5")
...
#   embd = np.load(glv + '.npy') 
  arguments,prem,hypo,sent1,sent2=data_processing(unique_words)
