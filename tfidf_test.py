...
def test_logistic():
    ...
    raw_dataset = list(train_examples("snli_1.0/snli_1.0_train.jsonl"))
    raw_dataset1 = list(test_examples("snli_1.0/snli_1.0_test.jsonl"))
    # raw_dataset1 = list(val_examples("snli_1.0/snli_1.0_dev.jsonl"))
    left = [s1 for _, s1, s2 in raw_dataset]
    right = [s2 for _, s1, s2 in raw_dataset]

    Y = np.array([B[l] for l, s1, s2 in raw_dataset])
    training=[left, right, Y]


    left = [s1 for _, s1, s2 in raw_dataset1]
    right = [s2 for _, s1, s2 in raw_dataset1]

    Y = np.array([B[l] for l, s1, s2 in raw_dataset1])
    test=[left, right, Y]
